...
class PlaceCommandSpec extends AnyFlatSpec with Matchers:
  ...
  it should "return Failure when placing on an occupied position" in:
    val occupiedState =
      PlaceCommand(validPos).execute(state).get
    ...
    cmd.execute(occupiedState) match
      case Failure(error) =>
        error.getMessage shouldBe GameMessages.occupiedPosition

      case Success(_) =>
        fail("Test fehlgeschlagen: Zug auf besetztes Feld hätte scheitern müssen!")
