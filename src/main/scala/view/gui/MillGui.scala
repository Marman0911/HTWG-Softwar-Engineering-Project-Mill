...
// MillGui ist jetzt eine CLASS, nicht mehr object
// Controller kommt per Dependency Injection rein
class MillGui(private var controller: IController) extends GameObserver:
  ...
  // Controller kommt per DI, Gui meldet sich als Observer an
  ...
  // startGame ersetzt startPlayerVsPlayer — kein neuer Controller mehr!
  ...
  private def playMove(position: String): Unit =
    controller.handleInput(position) match
      case Failure(error) =>   // FIX: Failure(error) Pattern
  ...
  // start() — wird von MillApp aufgerufen
  def start(): Unit =
    Swing.onEDT:
      blinkTimer.start()
      showPanel(mainMenuPanel)
  ...
  private class MillBoardPanel extends Panel:
    ...
    private def occupiedPoints: Set[(Int, Int)] =
      controller.boardViewModel.stones.map(stone => (stone.col / 5, stone.row / 2)).toSet
    ...
    private def startX: Int = (size.width - boardLength) / 2
    private def startY: Int = (size.height - boardLength) / 2
    private def pixelX(gridX: Int): Int = startX + gridX * boardLength / 6
    private def pixelY(gridY: Int): Int = startY + gridY * boardLength / 6
